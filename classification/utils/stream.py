...
def load_dataset(name='mnist', val_split=0.2, seed=0):
    ...
    train, val = torch.utils.data.random_split(train, [int(len(train) * (1 - val_split)), int(len(train) * val_split)])
...
def dataset_as_numpy(dataset):
    ...
    #
    batches = torch.utils.data.DataLoader(dataset, batch_size=len(dataset), shuffle=False, num_workers=1)
