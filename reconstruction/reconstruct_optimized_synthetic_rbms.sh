...
while [ $i -lt $N_RBMS ]; do
    ...
    python rbm_reconstruction.py ${DATASET} ${RBM_PATH}_${i} ${RBM_PATH}_${i} ${GAN_PATH} ${SAMPLED_EPOCH} -alpha 0

    # Reconstructing the test set with all pre-trained RBMs and sampled weights
    python rbm_reconstruction.py ${DATASET} ${RBM_PATH}_${i} ${RBM_PATH}_${i} ${GAN_PATH} ${SAMPLED_EPOCH} -alpha 1

    # Reconstructing the test set with all pre-trained RBMs using a linear combination of original and sampled weights
    python rbm_reconstruction.py ${DATASET} ${RBM_PATH}_${i} ${RBM_PATH}_${i} ${GAN_PATH} ${SAMPLED_EPOCH} -alpha 0.9
