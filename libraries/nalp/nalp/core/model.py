...
class Adversarial(Model):
    ...
    def fit(self, batches, epochs=100):
        ...
        samples_per_epoch = tf.zeros([0, 1, 1, self.input_dim[0]])
        ...
        for e in range(epochs):
            ...
            z = tf.random.normal([1, 1, 1, self.G.noise_dim])

            # Sampling from generator
            sampled_z = self.G(z)

            # Stacking tensors
            samples_per_epoch = tf.concat([samples_per_epoch, sampled_z], axis=0)
